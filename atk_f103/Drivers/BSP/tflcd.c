...
void lcd_id_get(void)
{
    ...
    lcddev.id = lcd_rd_data();  /* dummy read 无效数据*/
    lcddev.id = lcd_rd_data();  /* 读到0X00 */
    lcddev.id = lcd_rd_data();  /* 读取LCD id高位93 */
    lcddev.id <<= 8;
    lcddev.id |= lcd_rd_data(); /* 读取LCD id低位41 */
}
...
{
    uint8_t t, temp;
    uint8_t show_enable = 0;

    for (t = 0; t < len; t++)   /* 按总显示位数循环 */
    {
        temp = (num / lcd_pow(10, len - t - 1)) % 10;   /* 获取对应位的数字 */

        if (show_enable == 0 && t < (len - 1))   /* 没有使能显示,且还有位要显示 */
        {
            if (temp == 0)
            {
